...
public class Config extends ServerConfig {
    ...
    public static String getPopularVideosUrl(String token) {
        if ("".equals(token)) {
            return REST_HOST + "/getMostPopularVideos?server_key=" + SERVER_KEY + "&part=snippet,statistics,contentDetails&regionCode=" + REGION_CODE + "&catId=1&resultPerPage=" + RESULT_PER_PAGE;
        } else {
            return REST_HOST + "/getMostPopularVideos?server_key=" + SERVER_KEY + "&part=snippet,statistics,contentDetails&regionCode=" + REGION_CODE + "&catId=1&resultPerPage=" + RESULT_PER_PAGE + "&pageToken=" + token;
        }
    }
    ...
    public static String getTopViewedVideosUrl(String token) {
        ...
        if ("".equals(token)) {
            return REST_HOST + "/getMostViewedVideosBySearchAPI?server_key=" + SERVER_KEY + "&part=snippet&regionCode=" + REGION_CODE + "&catId=1&resultPerPage=" + RESULT_PER_PAGE;
        } else {
            return REST_HOST + "/getMostViewedVideosBySearchAPI?server_key=" + SERVER_KEY + "&part=snippet&regionCode=" + REGION_CODE + "&catId=1&resultPerPage=" + RESULT_PER_PAGE + "&pageToken=" + token;
        }
    }
